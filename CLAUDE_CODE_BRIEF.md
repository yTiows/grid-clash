...
**Phase 2 in progress (2026-08-01) — tournament economy rebuild:**
...
- **`scheduling.ts`'s `commitField` wired** (the other half of the demand-sizing task — `sizeFieldForDemand` was already wired via `admin-tournament-sizing.ts`). New `field_commit_mode` column on `tournaments` (`'fixed'` default, unchanged behavior; `'demand'` treats `field_size` as a ceiling). Reuses the existing `registration_opens_at`/`starts_at` columns as the registration window rather than adding new ones. `commit_tournament_field()` RPC (`20260801000002_demand_sized_field_commit.sql`, self-tested — logically verified; the happy-path assertion currently skips on the live project for lack of two phone-verified accounts, the same data-scarcity condition several pre-existing self-tests already report) atomically rewrites the committed field's money terms and flips it to `full`. New cron `/api/cron/commit-tournament-fields` (every 5 min, wired into `vercel.json`) finds demand-mode tournaments whose window closed, refunds under-floor contests in full (cancels) or snap-down overflow (latest registrants by `seat_number`, bracket formats only) via the existing `move_balance` idempotent path, then commits. Admin UI: a "Demand-sized" checkbox on `create-tournament-form.tsx`, requires `startsAt`. `database.types.ts` regenerated from the live schema to pick up all of the above.
- Not yet done: the rank-tiered tournament fee/margin schedule itself (hard stop — needs the person's confirmation before any settlement SQL changes, see the final handoff report for the actual proposal).
