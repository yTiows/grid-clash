...
export default function WagerPlayPage() {
  const router = useRouter()
  const params = useParams<{ challengeId: string }>()
  const challengeId = params.challengeId

  const { state, connectWager, submitMove, resign, disconnect } = useMatchSocket()

  useEffect(() => {
    if (challengeId && state.phase === "idle") {
      void connectWager(challengeId)
    }
    return () => {
      disconnect()
    }
    ...
    <div className="relative min-h-screen">
      <div className="container relative flex min-h-screen flex-col items-center justify-center py-12">
        <div className="w-full max-w-md">
          {(state.phase === "connecting" || state.phase === "wager_waiting") && (
            <div className="panel space-y-4 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Both stakes are already held. The match starts the moment they&apos;re here too.
              </p>
            ...
            <div className="panel space-y-4 p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="tabular money font-bold">${((state.stakeCents ?? 0) / 100).toFixed(2)}</div>
                </div>
              </div>
          ...
          {state.phase === "error" && (
            <div className="panel space-y-4 p-8 text-center">
              ...
              <Button onClick={() => router.push("/dashboard/wagers")}>Back to wagers</Button>
            </div>
          )}
        </div>
      </div>
